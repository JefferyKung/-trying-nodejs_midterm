...
const sql3 = `SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='railway' AND TABLE_NAME='Comments'`; //to query and check if table "Books" exists
...
const seedDB3 = () => {
  ...
  pool.query(
    `
      INSERT INTO Comments (commentID,Book_Id, Comment) VALUES
      (1, 1 , 'I feel so sorry about what I saw' );`,
    (err) => {
      if (err) {
        return console.error(err.message);
      }
      console.log("Successful creation of 1 comment");
    }
  );
};
